...
% TODO screenshot of messages in database for group and for users.
...
The user is able to login, join and leave groups, send messages to other users and groups, and read messages for them. Asynchronous communication is used, and so when requesting to join or leave a group the server may immediately reply with confirmation. 
...
% TODO
% Node messages are serialized POJOs, so all servers in the ring have to be running the exact same application.
% Assumes silence from a node is failure, and not just that traffic is delayed. Though the node can rejoin after absolute failure.
% Must know all possible nodes addresses when initializing the network, though it wouldnt be too difficult to introduce a mechanism for registering completetly nodes.
